...
public class EntityDamageEventActions {

    public static void register() {
        Registry.register(
                new ActionContainer<Temp>(
                        new IdentifierContainer(
                                OriginsBukkit.KEY, "action/listener/entity_damage_event/set_damage"
                        ), null,
                        (data, temp) -> {
        ...
        Registry.register(
                new ActionContainer<Temp>(
                        new IdentifierContainer(
                                OriginsBukkit.KEY, "action/listener/entity_damage_event/set_additional_damage"
                        ), null,
                        (data, temp) -> {
                            if (temp.getEvent() instanceof EntityDamageEvent) {
                                EntityDamageEvent event = (EntityDamageEvent) temp.getEvent();
